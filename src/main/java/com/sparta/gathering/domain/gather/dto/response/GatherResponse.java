...
public class GatherResponse {
    ...
    public GatherResponse(Gather gather) {
        ...
        this.likeCount = gather.getLikeCount();
        ...
        this.scheduleInfos = gather.getScheduleList().isEmpty() ? null :
                gather.getScheduleList().stream()
                        .map(schedule -> new ScheduleInfo(schedule.getId(), schedule.getScheduleTitle(),
                                schedule.getScheduleContent()))
                        .sorted(Comparator.comparing(ScheduleInfo::getId).reversed()) // ID 기준 내림차순 정렬
                        .collect(Collectors.toList());
        this.boardInfos = gather.getBoardList().isEmpty() ? null : gather.getBoardList().stream()
                .map(board -> new BoardInfo(board.getId(), board.getBoardTitle(), board.getBoardContent()))
                .sorted(Comparator.comparing(BoardInfo::getId).reversed()) // ID 기준 내림차순 정렬
                .collect(Collectors.toList());
